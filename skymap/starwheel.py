...
class StarWheel(BaseComponent):
    ...
    def do_rendering(self, settings_local, context):
        ...
        for dec in arange(-80, 85, 15):
            # Convert declination into radius from the centre of the skymap
        ...
        for line in open("raw_data/constellation_stick_figures.dat", "rt"):
            ...
            # If we're making a southern hemisphere skymap, we flip the sky upside down
            if is_southern:
                ra1 = -float(ra1)
                ra2 = -float(ra2)
                dec1 = -float(dec1)
